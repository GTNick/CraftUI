...
class Main extends PluginBase implements Listener {
	...
	case 1:
		...
		$inv->removeItem(Item::get(41,1,256));
...
break;
	case 2:
		...
		$level->addParticle(new LavaParticle($pos1));
		$inv->removeItem(Item::get(41,1,256));
		$this->getServer()->broadcastMessage("§6$name §8Crafted a §6Royle Helmet!");
		$inv = addItem(Item::get(283,1,1));
	}
	}else{
		$sender->sendMessage("§6 You don't have the items required to craft the Royale helmet");
	}
break;
				$form->setTitle("§6Craft");
				$form->setContent("§ecraft some sepcial items!");
				$form->addButton("§8Royale Sword", 1);
				$form->addButton("§8Prince Helmet", 2);
